...
export default function ItineraryPage() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      // No stored PDF — same pipeline as guide visualizer: API returns HTML, hidden iframe + html2canvas + jsPDF.
      if (!tourId) {
      ...
      const previewResp = await fetch(`${API_BASE_URL}/api/generate-styled-tour-pdf`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
      ...
      if (!previewResp.ok || !previewData.success || !previewData.previewHtml) {
        throw new Error(previewData.error || `Could not build styled PDF (${previewResp.status})`);
      }
      ...
      if (previewData.mapIncluded === false) {
        alert(`PDF downloaded, but the map was not included: ${previewData.mapIssue || 'Map generation failed.'}`);
      }
    } catch (error) {
  ...
  // Get price for preview (normalize legacy DB default 16 → €12 catalogue)
  const rawPreviewPdf =
    tourData?.price?.pdfPrice ?? tourData?.price_pdf ?? draftData?.tourSettings?.price?.pdfPrice;
  const rawPreviewCur =
    tourData?.price?.currency ?? draftData?.tourSettings?.price?.currency;
  const { amount: previewPdfPrice, currency: previewCurrency } = normalizeSelfGuidedDisplay(
    rawPreviewPdf,
    rawPreviewCur
  );
    ...
    >
          ...
          <div style={{ marginBottom: '40px' }}>
            ...
            <div style={{
              ...
              <div style={{
                ...
                <span style={{ fontSize: '17px', fontWeight: '500', color: '#111827', lineHeight: '1.3' }}>
              ...
              <div style={{
                ...
                <span style={{ fontSize: '24px', marginBottom: '8px' }}>🗺</span>
                <span style={{ fontSize: '17px', fontWeight: '500', color: '#111827', lineHeight: '1.3' }}>A ready-to-follow one-day route</span>
              </div>
              ...
              {tourHighlights.text3 && (
                <div style={{
                  ...
                  <span style={{ fontSize: '17px', fontWeight: '500', color: '#111827', lineHeight: '1.3' }}>{tourHighlights.text3}</span>
                </div>
              )}
              ...
              {tourHighlights.text4 && (
                <div style={{
                  ...
                  <span style={{ fontSize: '17px', fontWeight: '500', color: '#111827', lineHeight: '1.3' }}>{tourHighlights.text4}</span>
                </div>
              )}
              ...
              {tourHighlights.text5 && (
                <div style={{
                  ...
                  <span style={{ fontSize: '17px', fontWeight: '500', color: '#111827', lineHeight: '1.3' }}>{tourHighlights.text5}</span>
                </div>
              )}
              ...
              <div style={{
                ...
                />
                <span style={{ fontSize: '17px', fontWeight: '500', color: '#111827', lineHeight: '1.3' }}>An interactive map + downloadable PDF</span>
              </div>
            </div>
          </div>
      ...
      <div className="content-section" style={{ padding: '0 !important', paddingLeft: '0 !important', paddingRight: '0 !important' }}>
        ...
        {previewOnly && !isPaid && (() => {
          ...
          // Get prices (normalize legacy 16 in DB → €12 for display)
          const { amount: pdfPrice, currency } = normalizeSelfGuidedDisplay(
            tourData?.price?.pdfPrice ?? tourData?.price_pdf,
            tourData?.price?.currency ?? tourData?.currency
          );
